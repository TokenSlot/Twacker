<?php
    ...
    $twack_link = $_GET['twack_link'];
    $twacks = "SELECT twack_tbl.*,user_tbl.id,user_tbl.username user FROM twack_tbl INNER JOIN user_tbl ON twack_tbl.author=user_tbl.id WHERE twack_link=$twack_link ORDER BY date_post DESC";
            ...
            <div class="card-body pt-3 pr-3 pl-3 pb-0">
                ...
                <ul class="list-inline mb-2 border-top pt-1">
                    ...
                    <div class="d-none d-md-inline">
                        <li class="list-inline-item border-left pl-3">
                            <a name="<?=$row['twack_id']?>" onclick="show_edit_modal('<?=$row['twack_id']?>')" class="btn btn-link m-0 p-0 text-secondary twack-deco" data-toggle="modal" data-target="#editTwackModal">
